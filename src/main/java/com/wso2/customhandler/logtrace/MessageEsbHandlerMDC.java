...
public class MessageEsbHandlerMDC extends AbstractSynapseHandler 
{
	...
	public boolean handleRequestOutFlow(MessageContext synCtx) {
		...
		try {
			...
			synCtx.setProperty("MicroApiPath", LogTrackUtil.getToHTTPAddress(synCtx));
			synCtx.setProperty("MicroPayload", LogTrackUtil.getHTTPRequestPayload(synCtx));
	...
	public boolean handleResponseInFlow(MessageContext synCtx) {
		...
		try {
			...
			MDC.put("MicroApiPath", synCtx.getProperty("MicroApiPath"));
			MDC.put("MicroPayload", synCtx.getProperty("MicroPayload"));
			MDC.put("MicroResponseCode", LogTrackUtil.getHTTPStatusMessage(synCtx));
			if (!LogTrackUtil.getHTTPStatusMessage(synCtx).equals("200"))
				MDC.put("MicroPayloadResponse", LogTrackUtil.getHTTPRequestPayload(synCtx));
			
			long backendTime = 0, backendEndTime = 0, backendStartTime = 0;
	...
	public boolean handleResponseOutFlow(MessageContext synCtx) {
		...
		try {
			...
			if (API_NAME != null)
				MDC.put("ApiName", API_NAME);
			
			if (CONTEXT != null)
				MDC.put("ApiContext", CONTEXT);
			
			if (SUB_PATH != null)
				MDC.put("ApiPath", SUB_PATH);

			if (HTTP_METHOD != null)
				MDC.put("Method", HTTP_METHOD);

			if (HTTP_RESPONSE_STATUS_CODE != null)
				MDC.put("ResponseCode", HTTP_RESPONSE_STATUS_CODE);

			MDC.put("ResponseTime", responseTime);
			MDC.put("BackendTime", backendTime);
			MDC.put("ServiceTime", serviceTime);

			if (ERROR_CODE != null)
				MDC.put("ErrorCode", ERROR_CODE);

			if (ERROR_MESSAGE != null)
			 MDC.put("ErrorMessage", ERROR_MESSAGE);
			
			log.auditLog("API Transaction Details:" + "API_NAME: " + API_NAME + ",HTTP_METHOD: " + HTTP_METHOD
					+ ", CONTEXT: " + CONTEXT + ",FULL_REQUEST_PATH" + FULL_REQUEST_PATH + ",SUB_PATH: " + SUB_PATH
					+ ", HTTP_RESPONSE_STATUS_CODE: " + HTTP_RESPONSE_STATUS_CODE + ", RESPONSE_TIME: " + responseTime
					+ ", BACKEND_TIME: " + backendTime + ", SERVICE_TIME: " + serviceTime + ", ERROR_CODE: "
